...
class PyHopPlanner(base.BaseModule):
    ...
    def run(self, cycle, verbose = 2):
        ...
        try:
            # print(self.mem.get(self.mem.GOAL_GRAPH))
            # goal.getplan
            midcaPlan = self.mem.get(self.mem.GOAL_GRAPH).getMatchingPlan(goals)
            # print("From mem goal graph get plan " + str(midcaPlan))
        except AttributeError:
            midcaPlan = None
        ...
        if midcaPlan:
            ...
            valid = world.plan_correct(midcaPlan)
            print("Is old plan vald " + str(valid))
        ...
        if not midcaPlan:
            ...
            try:
                ...
                print("State for replanning " + str(pyhopState))
            except Exception as e:
                print("error: has occurred at pyhopState" + str(e))
                print "Could not generate a valid pyhop state from current world state. Skipping planning"

            try:
                pyhopTasks = self.pyhop_tasks_from_goals(goals,pyhopState)
                print("here in tasks")
                print("Tasks from goals for replanning " + str(pyhopTasks))
            except Exception as e:
                print("error: has occurred at pyhopTasks " + str(e))
                print "Could not generate a valid pyhop task from current goal set. Skipping planning"
            ...
            try:
                ...
                print("Here in replanning ......")
                self.mem.set(self.mem.PLANNING_COUNT, 1+self.mem.get(self.mem.PLANNING_COUNT))
                print("here")
                pyhopPlan = pyhop.pyhop(pyhopState, pyhopTasks, verbose = 0)
                print("It creates a pyhop plan " + str(pyhopPlan))
            except Exception as e:
                print("error: has occurred at pyhopPlan " + str(e))
                traceback.print_exc()
                pyhopPlan = None
            ...
            print("Midca Plan is " + str(midcaPlan))
            if verbose >= 1:
                print "Planning complete."
            if verbose >= 2:
                print "Plan: "#, midcaPlan
...
class PyHopPlanner_temporary(base.BaseModule):
	...
	try:
        ...
        print("Midca plan" + str(midcaPlan) +"\n")
        if midcaPlan:
            if verbose >= 2:
                print "Old plan retrieved. Checking validity...",
            valid = world.plan_correct(midcaPlan)
