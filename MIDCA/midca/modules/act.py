...
class GoalInferenceAct(base.BaseModule):
    ...
    def run(self, cycle, verbose = 2):
        ...
        if plan != None:
            ...
            if action != None and action != []:
                operation = action[0].op
                arguments = action[0].args
                print(arguments)
            
                print("No goal or a plan was found, So performing  ")
                state = util.pyhop_state_from_world(world)
                print "Selected action", action
                if operation == "pickup":
                    new_state = operators.pickup(state,arguments[0])
                    self.mem.set(self.mem.STATES, new_state)
                if operation == "putdown":
                    new_state = operators.putdown(state,arguments[0])
                    self.mem.set(self.mem.STATES, new_state)
                if operation == "stack":
                    print(arguments[0])
                    print(arguments[0][0])
                    new_state = operators.stack(state, arguments[0], arguments[1])
                    self.mem.set(self.mem.STATES, new_state)
                if operation == "unstack":
                    new_state = operators.unstack(state, arguments[0][0], arguments[0][1])
                    self.mem.set(self.mem.STATES, new_state)
            else:
                if verbose >= 1:
...
class SimpleAct(base.BaseModule):
    ...
    To have MIDCA perform multiple actions in one cycle,
     simple add several actions to mem[mem.ACTIONS][-1].
      So mem[mem.ACTIONS][-1][0] is the first action taken,
       mem[mem.ACTIONS][-1][1] is the second, etc.
    '''
        ...
        if plan != None:
            ...
            if actions != None and actions != []:
                for action in actions:
                    ...
                    print(args[0])
                    if operation == "pickup":
                        new_state = operators.pickup(state,args[0])
