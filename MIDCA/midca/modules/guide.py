...
from midca.plans import Action
from midca.worldsim import stateread
import copy,csv
import random
from midca.modules.monitors import Monitor
from threading import Thread
from PractNLPTools.practnlptools.tools import Annotator
...
'''
    MIDCA module that allows user to speak to the agent,
    the agent will translate the speech to text.
    the text is interpreted --> hardcore mapping.
        - use semantic role labeling here
        then rest of midca cycle is continued.
'''
class GoalInference(base.BaseModule):

    def __init__(self, world):
        self.world = world

    def synonymns(self, words):
        synsets = []
        for word in words:
            ...
            if syns!= []:
                for syn in syns:
                    ...
                    if name in synsets:
                        ...
                        synsets.append(str(name))
            else:
                synsets.append(word)
        return synsets

    def semantic_role_labels(self, speech, operator_keywords):
        annotator = Annotator()
        srls = annotator.getAnnotations(speech)['srl']
        if srls != []:
            for srl in srls:
                verb = str(srl["V"])
                #print(verb)
                # (verb, arg1, arg2 )
                if verb in operator_keywords:
                    return srl
                else:
                    # the verb found for srl is not in operator_keywords
                    return False
        else:
            return False
                #
    ...
    def preprocess(self, instruction):
        return instruction.replace("\n","").replace("\r", "").lower().replace(".","").replace(","," ")
    ...
    def check_preconditons(self, state,  operator, args):
        # pull the preconditions
        # not have to be a function pass a domain
        if operator == "stack":
            print(args[0])
            print(state.pos[args[0]])
            print(" returns flase in this scenario .....")
            return (state.pos[args[0]] == 'in-arm' and state.clear[args[1]] == True)
        if operator == "unstack":
            return (state.pos[args[0]] == args[1] and args[1] != 'table' and state.clear[args[0]] == True and state.holding == False)
        if operator == "pickup":
            return (state.pos[args[0]] == 'table' and state.clear[args[0]] == True and state.holding == False)
        if operator =="putdown":
            return (state.pos[args[0]] == 'in-arm')

    def operator_helper(self, speech, keywords, current_state, color_map, cycle, operator,  predicate, option):
        goals_entered = []
        srl = self.semantic_role_labels(speech, keywords)
        print(srl)
        if srl == [] or srl == False:
            print("Couldn't understand the instruction: " + str(speech))
        else:
            # srl worked.
            args = []
            top_blk = color_map[str([i for i in str(srl['A1']).lower().split(' ') if self.check_color(i)][0])]
            print(top_blk)
            print(operator)
            args.append(top_blk)
            bttm_blk = " "
            if operator == "stack" or operator == "unstack":
                bttm_blk = color_map[str([i for i in str(srl['A2']).lower().split(' ') if self.check_color(i)][0])]
                #print(bttm_blk)
                args.append(bttm_blk)

            flag = self.check_preconditons(current_state, operator, args)
            print("Flag point = " + str(flag))
            if flag:
                 if operator == "stack" or operator == "unstack":

                     action = Action(operator, top_blk, bttm_blk)
                     self.mem.add(self.mem.ACTIONS, [action])
                 else:
                      #print("Here adding operator "+ operator)
                      action = Action(operator, top_blk)
                      self.mem.add(self.mem.ACTIONS, [action])

                 trace = self.mem.trace
                 if trace:
                     trace.add_module(cycle,self.__class__.__name__)
                     trace.add_data("USER Speech - Action", [action])

            else:
                # most likely goal
                print("creating a goal......")
                goal = goals.Goal(*args, predicate = predicate, negate = option)
                ...
                if trace:
                    trace.add_module(cycle,self.__class__.__name__)
                    trace.add_data("USER's MOST LIKELY GOAL INFERENCE", goals_entered)
    ...
    def run(self, cycle, verbose = 2):
        ...
        # stack_keywords = self.synonymns(["stack", "on", "put", "place", "placed"])
        stack_keywords = ['stack', 'batch', "on", "put", "place", "placed", 'push-down_list', 'smokestack', 'push-down_storage', 'on', 'along', 'put_option', 'put', 'frame', 'invest', 'place', 'arrange', 'topographic_point', 'stead', 'home', 'position', 'seat', 'plaza', 'space', 'rate', 'locate', 'target', 'identify', 'set', 'station', 'located', 'placed']
        keywords.extend(stack_keywords)
        # print(stack_keywords)

        # pick_keywords=  self.synonymns(["pick", "pickup", "hold", "grab", "get"])
        pick_keywords=  ["pick", "pickup", "hold", "grab", "get", 'choice', 'picking', 'cream', 'woof', 'pick', 'blame', 'clean', 'foot', 'pluck', 'peck', 'nibble', 'pickup', 'tone_arm', 'cartridge', 'clasp', 'appreciation', 'hold', 'delay', 'detention', 'handle', 'cargo_area', 'keep', 'have', 'deem', 'harbor', 'restrain', 'retain', 'bear', 'accommodate', 'prevail', 'contain', 'reserve', 'defend', 'oblige', 'defy', 'apply', 'control', 'halt', 'carry', 'declare', 'agree', 'grab', 'catch', 'snap_up', 'get', 'become', 'receive', 'arrive', 'bring', 'experience', 'pay_back', 'induce', 'grow', 'contract', 'make', 'drive', 'scram', 'draw', 'perplex', 'get_down', 'suffer', 'beget']
        keywords.extend(pick_keywords)
        #print(pick_keywords)

        # unstack_keywords =  self.synonymns(["unstack", "clear", "remove", "replace"])
        unstack_keywords = ['unstack', 'clear', 'open', 'unclutter', "remove", "replace", 'clear_up', 'authorize', 'pass', 'net', 'gain', 'acquit', 'clean', 'well-defined', 'absolved', 'clearly', 'remove', 'get_rid_of', 'take_out', 'absent', 'murder', 'replace', 'supplant', 'substitute']
        keywords.extend(unstack_keywords)
        # print(unstack_keywords)

        putdown_keywords = ["putdown"]
        keywords.extend(putdown_keywords)
        print(putdown_keywords)

        color_map = self.mapping()
        #print(color_map)


        while True:
            speech = raw_input("Please input an instruction. Otherwise, press enter to continue\n")
            if not speech:
                return "continue"

            current_state =  util.pyhop_state_from_world(self.world)

            '''
            if speech contains any of the stack keywords
                it is possible that the speech is a goal or action
                if action/task: all the preconditions are met
                if they are met add the action item
                else plan for a stackng goal.
           '''

            if any (stack_keyword in speech for stack_keyword in stack_keywords):
                operator = "stack"
                predicate = "on"
                option = False
                self.operator_helper(speech, stack_keywords, current_state, color_map, cycle, operator, predicate,option)
            elif any (unstack_keyword in speech for unstack_keyword in unstack_keywords):
                operator = "unstack"
                predicate = "on"
                option = True
                self.operator_helper(speech, unstack_keywords, current_state, color_map, cycle, operator, predicate, option)
            elif any (pick_keyword in speech for pick_keyword in pick_keywords):
                operator = "pickup"
                option = False
                predicate = "holding"
                self.operator_helper(speech, pick_keywords, current_state, color_map, cycle, operator, predicate, option)
            elif any (putdown_keyword in speech for putdown_keyword in putdown_keywords):
                operator = "putdown"
                option = True
                predicate = "holding"
                self.operator_helper(speech, putdown_keywords, current_state, color_map, cycle, operator, predicate, option)
