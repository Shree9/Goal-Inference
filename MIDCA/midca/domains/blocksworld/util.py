...
def pyhop_tasks_from_goals(goals, pyhopState):
    print("here in task decomposition")
    alltasks = []
    blkgoals = pyhop.Goal("goals")
    blkgoals.pos = {}
    for goal in goals:
        ...
        print("The Goal ")
        print(goal)
        print("----->goal args = " + str(goal.kwargs))
        if 'predicate' in goal.kwargs:
            predicate = str(goal.kwargs['predicate'])
        elif 'Predicate' in goal.kwargs:
            predicate = str(goal.kwargs['Predicate'])
        ...
        args = [str(arg) for arg in goal.args]
        print("The args " + str(args))
        if args[0] == predicate:
            args.pop(0)
        if predicate == "on":
            print("here in on ")
            blkgoals.pos[args[0]] = args[1]
            print(blkgoals.pos.keys())
            print(blkgoals.pos.values())
            print("after on")
        elif predicate == 'on-table':
    ...
    print(alltasks)
    print("end of task decomposition")
    return alltasks
