...
class GoalGraph:
    ...
    '''
    A graph that maintains a partial ordering of goals.
    Note that, at present,
        cycle checking is not complete,
        so partial orderings can be created that would never allow a goal to be accomplished.
    The single constructor argument
        gives a function that takes two goals as input
        and should return a +/- value indicating precedence.
        If goal1 should be achieved before goal2,
        goalCompareFunction(goal1, goal2) < 0.
    '''
    ...
    # checks if two goals are the same
